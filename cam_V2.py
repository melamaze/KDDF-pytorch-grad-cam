...
if __name__ == '__main__':
    ...
    # Class
    keys = ['backward', 'bed', 'bird', 'cat', 'dog', 'down', 'eight', 'five', 'follow', 'forward', 'four', 'go', 'happy', 'house', 'learn', 'left', 'marvin', 'nine', 'no', 'off', 'on', 'one', 'right', 'seven', 'sheila', 'six', 'stop', 'three', 'tree', 'two', 'up', 'visual', 'wow', 'yes', 'zero']
    values = [i for i in range(35)]
    my_dict = {k : v for k, v in zip(keys, values)}
    # Read global model
    global_model = ResNet18() # remember to change package/FL/resnet.py line: 72 -> num_class = 35
    PATH = './model/resnet_15_0.2_V2_start.pth'
    global_model.eval().cuda()
    global_model.load_state_dict(torch.load(PATH, map_location='cuda:0'))
    # Read examine model
    model = ResNet18()
    PATH = './student_model/student_resnet_15_0.2_V2_start.pth'
    model.eval().cuda()
    ...
    for path in glob.glob(folder):
        ...
        probability = torch.tensor([0.0 for i in range(35)]).cuda()
